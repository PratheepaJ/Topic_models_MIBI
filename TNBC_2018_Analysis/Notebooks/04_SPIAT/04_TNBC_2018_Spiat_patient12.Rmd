...
```{r}
spe_1 <- spe[, spe$sample_id %in% c("Sample_12")]
...
We want to visualize the phenotype in each tissues using categorical dot plot.
```{r}
...
```{r}
p_cells_12 <- calculate_cell_proportions(spe_object = spiat_sample1,
                           reference_celltypes = NULL,
                           celltypes_to_exclude = NULL,
                           feature_colname = "Phenotype",
                           plot.image = FALSE)

cell_prop_p12 <- (p_cells_12
...
```{r}
plot_cell_percentages(cell_proportions = p_cells_12,
...
formatted_structure_12 <- define_structure(spe_object = formatted_distance,
                                        cell_types_of_interest = immune_cells,
                                        feature_colname = "Phenotype",
                                        n_margin_layers = 5)

categories <- unique(formatted_structure_12$Structure)
```

We now plot and color these structure categories.
```{r}
tis_structure_p12 <- plot_cell_categories(spe_object = formatted_structure_12,
...
```{r}
ggpubr::ggarrange(
  tis_structure_p4, tis_structure_p12,
  labels = c("A", "B"),
  widths = 10, heights = 5,
  ncol = 2
)

ggsave(filename = "tiss_structure_plots.eps", device = "eps",
       width = 14, height = 6,
