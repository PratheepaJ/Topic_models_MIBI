...
#ggsave("Tissue_visualize_patient4.jpeg", path = "/Users/henzhwang/Desktop/", 
#       width = 9, height = 7)
```
...
```{r}
p_cells_4 <- calculate_cell_proportions(spe_object = spiat_sample1,
                           reference_celltypes = NULL,
                           celltypes_to_exclude = NULL,
                           feature_colname = "Phenotype",
                           plot.image = FALSE)

cell_prop_p4 <- (p_cells_4
  |> dplyr::mutate(Cell_type = forcats::fct_reorder(Cell_type, Percentage))
  |> ggplot2::ggplot() 
  + geom_bar(aes(x = Cell_type, y = Percentage),
...
# ggsave(filename = "sample_cellType_plot.eps",
#        plot = sample_cellType,
#        width = 8, height = 6,
...
#    cell_prop_p4, cell_prop_p12,
#    labels = c("A", "B"),
#    widths = 10, heights = 6) +
#   theme(text = element_text(size = 18)) 
# 
# ggsave(filename = "Cell_proportion_4_12.eps", plot = arrange, device = "eps",
#        width = 10, height = 5,
#        path = here::here("Output", "Figures"))
```
...
```{r}
plot_cell_percentages(cell_proportions = p_cells_4,
...
distance <- calculate_pairwise_distances_between_celltypes(
  spe_object = spiat_sample1,
  cell_types_of_interest = c("B", "CD4 T", "Epithelial", "Mesenchymal", "Other"),
  feature_colname = "Phenotype")
```

```{r}
pair_violin_p4 <- plot_cell_distances_violin(distance)
```

```{r}
# ggpubr::ggarrange(
#   ggpubr::ggarrange(cell_prop_p4, cell_prop_p12,
#                     labels = c("A", "B"),
#                     widths = 10, heights = 3),
#   ggpubr::ggarrange(pair_violin_p4,
#                     labels = c("C"),
#                     widths = 10, heights = 8),
#   nrow = 2,
#   heights = c(3, 6)
# )
# 
# ggsave(filename = "cellCount_pairDist_plots.eps", device = "eps",
#        width = 10, height = 9,
#        path = here::here("Output", "Figures"))
```
...
formatted_structure_4 <- define_structure(spe_object = formatted_distance,
                                        cell_types_of_interest = immune_cells,
                                        feature_colname = "Phenotype",
                                        n_margin_layers = 5)

categories <- unique(formatted_structure_4$Structure)
```

We now plot and color these structure categories.
```{r}
tis_structure_p4 <- plot_cell_categories(spe_object = formatted_structure_4,
                     feature_colname = "Structure") + ggplot2::ggtitle("")
tis_structure_p4

#ggsave("Tissue_structure_patient4.jpeg", path = "/Users/henzhwang/Desktop/",
#       width = 9, height = 7)
```
...
clusters <- identify_neighborhoods(spe_object = spiat_sample1,
                                   method = "hierarchical",
                                   #cell_types_of_interest = immune_names,
                                   cell_types_of_interest = c(tumor_names, immune_names, nonimmune_names),
                                   radius = 50,
