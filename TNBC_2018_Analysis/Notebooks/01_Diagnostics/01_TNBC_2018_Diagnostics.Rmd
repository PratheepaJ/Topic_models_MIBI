---
title: "01_TNBC_2018_Diagnostics"
...
```{r}
#TNBC <- readr::read_csv("Data/MIBI-TNBC_count_output.csv")
...
Summary of count of cells in each sample. 
Sample 33 has the minimum of 1716 cells, sample 30 has the maximum of 9886 cells. 
```{r}
TNBC_count <- TNBC %>% dplyr::count(sample_id)
TNBC_count
hist(TNBC_count$n,
     main = "Histogram of cell counts in each sample",
     xlab = "Unique sample_id", ylab = "Counts",
     breaks = 25, xlim = c(1000, 10000), ylim = c(0, 8))
summary(TNBC_count)
```
Summary of count of cells in each cluster. 
cluster 10 has the least number of $783$ cells, and cluster113 has the most number of $4382$ cells.
```{r}
TNBC_cluster_count <- TNBC %>% dplyr::count(cluster_id)
TNBC_cluster_count
hist(TNBC_cluster_count$n,
     main = "Histogram of cell counts in each cluster",
     xlab = "Unique sample_id", ylab = "Counts",
     breaks = 30, xlim = c(500, 4500), ylim = c(0, 20))
summary(TNBC_cluster_count)
```
...
# n_(1,2,3,4,5,6,7) are the unique values of the metadata in one sample
TNBC %>% 
  group_by(sample_id) %>% 
