...
## Sample-CellType Histogram and Barplot

We want to plot histogram and bar plot for sample and cell types. We first want to plot the bar plot of total count of each cell type count in all samples.
...
# ggsave(filename = "CellType_count_all.eps", device = "eps",
#        plot = CellType_count_all,
#        width = 8, height = 6,
#        path = here::here("Output", "Figures"))
```

Next, we want to plot the unqieu cell type count in each sample
...
sample_cellType <- (
  ...
  |> dplyr::summarise(unique_mmCount = length(unique(mm)))
  ...
  |> ggplot2::ggplot(aes(x = sample_id, y = unique_mmCount))
  + geom_bar(stat = "identity", width = 0.7
             ,fill="#f68060") 
             #alpha=.8, width=.7)
  #+ coord_flip()
  + xlab("Sample Identity") + ylab("Unique Cell Type Coount")
  + ggtitle("")
  + theme(text = element_text(size = 18)) 
...
We now want to combine two plots together

```{r}

arrange <- ggpubr::ggarrange(
   CellType_count_all, sample_cellType,
   labels = c("A", "B"),
   widths = 10, heights = 6) +
  theme(text = element_text(size = 18)) 
...
TNBC %>% 
  ...
  ggplot(mapping = aes(x = mm, y = sample_id, fill = count)) +
    ...
    labs(x = "Cell Type", y = " Sample Identity", title = "Cell Type-Sample Distribution of Triple Negative Breast Cancer")+
    ...
    theme(legend.position = "right", legend.direction = "vertical",
        ...
        legend.text = element_text(colour = "grey40", size = 10, face = "bold"),
        ...
        plot.title = element_text(colour = "grey40", hjust = 0, size = 10, face = "bold")
     )+
